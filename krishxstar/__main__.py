...
import config
from krishxstar import LOGGER, assistant, bot, call_py
from krishxstar.modules import ALL_MODULES
...
@_flask.route("/")
def _home():
    return "❍ KrishXStarMusic is running made by KRISH X STAR CODER 💕", 200
...
if __name__ == "__main__":
    ...
    # 1. MongoDB
    try:
        from krishxstar.database import start_mongo
        ok = start_mongo()
        if ok:
    ...
    for mod in ALL_MODULES:
        try:
            importlib.import_module(f"krishxstar.modules.{mod}")
    ...
    try:
        import krishxstar.core.call  # noqa: F401
    except Exception as e:
    ...
    # 11. Watchdog
    from krishxstar.core.watcher import watchdog
    loop.create_task(watchdog())
    LOGGER.info("Watchdog started")

    LOGGER.info(" KrishXStarMusic is running")
    ...
    LOGGER.info("✧ KrishXStarMusic stopped ✧")
