...
import config
from krishxstar import LOGGER, bot, call_py
from krishxstar.utils.formatters import fmt_time, parse_dur, progress_bar, short
from krishxstar.utils.youtube import resolve_stream
...
async def _ensure_vc(chat_id: int) -> None:
    ...
    from krishxstar import assistant
    import random
    try:
...
async def play_song(chat_id: int, message: Message, song: dict) -> None:
    ...
    loading_text = f"<b>❍ Loading :</b> {short(song['title'])}"
    try:
    ...
    for attempt in range(2):
        try:
            ...
            await bot.send_message(
                ...
                "<i>Make sure assistant is in the group and VC can be started.</i>",
                parse_mode=ParseMode.HTML,
            )
            return
    ...
    # ── DB: track play count ──────────────────────────────────────────────────
    try:
        from krishxstar.database import add_served_chat, add_served_user, increment_play_count
    ...
    await _update_progress(chat_id, message, time.time(), total, caption)
